...
int alt_up_pixel_buffer_dma_move_buffers(alt_up_pixel_buffer_dma_dev *pixel_buffer)
/* This function swaps the front and back buffers. At the next refresh cycle the back buffer will be drawn on the screen
 * and will become the front buffer. */
{
	register unsigned int x;
	register unsigned int value_read;
	register unsigned int limit_x;
	register unsigned int limit_y;

	register unsigned int addr_back;
	register unsigned int addr_front;

	addr_back = pixel_buffer->back_buffer_start_address;
	addr_front = pixel_buffer->buffer_start_address;
	...
	for (x = 0; x<limit_y; x = x + 4)
	{
		value_read = IORD_32DIRECT(addr_back,x);
		IOWR_32DIRECT(addr_front, x, value_read);
//		printf("addr_front = %d\n", addr_front);
//		printf("addr_back = %d\n", addr_back);
//		printf("value_read = %d\n", value_read);
	}
