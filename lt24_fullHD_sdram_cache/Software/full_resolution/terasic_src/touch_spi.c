...
void evaluate_swipe(TOUCH_HANDLE pHandle, SWIPE *touch_swipe, POINT *pt){
	...
	if(touch_pending){ 							//prima della chiamata di questa funzione � stato registrato un tocco,
		...
		if(touch_swipe->point_valid){ 			//tocco precendente era valido, quindi calcolo i parametri dello swipe
 												//sarebbe da passargli direttamente il campo della struttura
			touch_swipe->first_touch = FALSE;
			...
			touch_swipe->end_of_swipe = FALSE; //false	//ho nuovo tocco, quindi fermo end_of_swipe
